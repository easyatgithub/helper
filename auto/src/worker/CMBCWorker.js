...
export default class {
  ...
  async fillTransferFromDifferentBank() {
    //await this.driver.wait(until.elementLocated(By.id("Amount")), 15 * 1000).sendKeys(this.task.requestAmount);
    //await this.driveruntil.elementLocated(By.css("input[v-model='PayeeAcName']"))).sendKeys(this.task.receiverName);
    //await this.driver.wait(until.elementLocated(By.id("PayeeAcNo")), 15 * 1000).sendKeys(this.task.bank.cardNumber);

    // Amount
    await sendKeys(this.driver.wait(until.elementLocated(By.id("Amount"), 5 * 1000)), {
    ...
    // Account
    await sendKeys(this.driver.wait(until.elementLocated(By.id("PayeeAcNo"), 5 * 1000)), {
  ...
  async fillTransferFromSameBank() {
    ...
    // Amount
    await sendKeys(this.driver.wait(until.elementLocated(By.id("amountInput"), 5 * 1000)), {
      text: Number.parseFloat(this.task.requestAmount).toFixed(2),
      replaceRule: /\,/g,
    });
    // Name
