...
export default class {
  ...
  async fillTransferFromSameBank() {
    ...
    // Name
    await sendKeys(this.driver.wait(until.elementLocated(By.id("txt_payeename_455"), 5 * 1000)), {
      text: this.task.receiverName,
    });
    // Amount
    await sendKeys(this.driver.wait(until.elementLocated(By.id("txt_transamount_471"), 5 * 1000)), {
  ...
  async fillTransferFromDifferentBank() {
    ...
    // Name
    await sendKeys(this.driver.wait(until.elementLocated(By.id("txt_payeename_1600"), 5 * 1000)), {
      text: this.task.receiverName,
    });
  ...
  async waitUntilBankSelected() {
    ...
    while (retryTimes >= 0) {
      try {
        ...
        await this.driver
          .wait(until.elementLocated(By.id("txt_transinaccparent_1601")), 1 * 1000)
          .sendKeys(this.task.bank.cardNumber);
      } catch (error) {
        if (error.name === "JavascriptError" || error.name === "TimeoutError") {
          setLog({
            level: "warn",
