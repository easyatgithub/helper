...
export default class {
  ...
  async fillTransferFrom() {
    try {
      ...
      await sendKeys(this.driver.wait(until.elementLocated(By.id("recAccountName"), 5 * 1000)), {
        text: this.task.receiverName,
      });

      await sendKeys(this.driver.wait(until.elementLocated(By.id("recAccountshow"), 5 * 1000)), {
        text: this.task.bank.cardNumber,
        replaceRule: regexHelper.removeSpace,
      });
      await sendKeys(this.driver.wait(until.elementLocated(By.id("transferSum"), 5 * 1000)), {
      ...
      if (!bank) {
        ...
        await sendKeys(this.driver.wait(until.elementLocated(By.id("queryBank"), 5 * 1000)), {
          text: bankName,
        });
        // 查询
        await executeJavaScript(
