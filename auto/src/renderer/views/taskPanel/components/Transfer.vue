<template>
  <div class="transfer__container">
    <div class="transfer__last-task-container">
      <div class="last-task__header">Last Task</div>
      <div class="last-task__body">
        <div class="detail">
          ...
          <div class="detail-data">
            <div v-if="lastSelectedTask">{{ lastSelectedTask.id }}</div>
            <div v-if="lastSelectedTask && lastSelectedTask.bank">{{ card.currentDetail.channelGroup || ' - ' }}</div>
            <div v-if="lastSelectedTask">{{ lastSelectedTask.merchantName }}</div>
            <div v-if="lastSelectedTask ">{{ lastSelectedTask.receiverName }}</div>
            <div v-if="lastSelectedTask && lastSelectedTask.bank">{{ lastSelectedTask.bank.englishName }}</div>
            <div v-if="lastSelectedTask && lastSelectedTask.bank">{{ lastSelectedTask.bank.cardNumber }}</div>
            <div v-if="lastSelectedTask">{{ lastSelectedTask.requestAmount }}</div>
            <div v-if="lastSelectedTask">{{ lastSelectedTask.toolStatus }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div class="current-task__header">Current Task</div>
      <div class="current-task__body">
        <div class="detail">
          ...
          <div class="detail-data">
            <div v-if="selectedTask">{{ selectedTask.id }}</div>
            <div v-if="selectedTask">{{ card.currentDetail.channelGroup || ' - ' }}</div>
            <div v-if="selectedTask">{{ selectedTask.merchantName }}</div>
            <div v-if="selectedTask ">{{ selectedTask.receiverName }}</div>
            <div v-if="selectedTask && selectedTask.bank">{{ selectedTask.bank.englishName }}</div>
            <div v-if="selectedTask && selectedTask.bank">{{ selectedTask.bank.cardNumber }}</div>
            <div v-if="selectedTask">{{ selectedTask.requestAmount }}</div>
          </div>
        </div>
        <div class="workflow">
          <table class="workflow-table">
            ...
            <tbody>
              ...
              >
                ...
                <td class="transfer__task-workflow-name">{{ flow.name }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      >
        <div class="prompt">Please select carefully of the result ...</div>
        <div class="operator">
          <el-button
            size="mini"
            type="success"
...
export default {
  ...
  data() {
    return {
      ...
      isHandlingToConfirm: false,
      isHandlingReassign: false,
    };
  },
  computed: {
    ...mapGetters(["card", "task", "worker"]),
    lastSelectedTask() {
