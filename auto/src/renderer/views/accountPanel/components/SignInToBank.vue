...
export default {
  ...
  methods: {
    ...
    async checkIsSignedIn() {
      ...
      try {
        await this.$store.dispatch("CheckIfLoginSuccess");
        this.$message({ type: "success", message: "You have already loggin" });
        this.handleSignInSuccess();
      } catch (error) {
        this.checkResult.type = "error";
        this.checkResult.message = "Fail to verify login status, are you sure login is completed successfully?";
        return;
      } finally {
